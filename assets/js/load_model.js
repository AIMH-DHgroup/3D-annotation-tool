...
import {CSS2DObject, CSS2DRenderer} from '../lib/renderers/CSS2DRenderer.js';
import {CSS3DObject, CSS3DRenderer} from '../lib/renderers/CSS3DRenderer.js';
...
async function createZip(chosenModel, username) {
    try {
        ...
        const modelFileBlob = await loadLocalFile("./php/models/" + username + "/" + modelName);
...
if (!chosenModel) throw new Error('No model found.');
    ...
    if (extension === '.zip') {
        ...
        try {
            ...
            const zipFileBlob = await loadLocalFile("./php/models/" + username + "/" + modelName);
    ...
    async function initXR() {
        ...
        await loader.load('./php/models/' + username + '/' + modelName.slice(0, -4) + '.glb', async function(gltf) {
            ...
            await fetch('./storage/json/' + username + '/' + modelName.slice(0, -4) + '.json?nocache=' + new Date().getTime())
            ...
            const formData = new FormData();
            // Pass only the relative path to the GLB file within the php/models directory.
            // removeGLB.php resolves this relative path against its own models directory.
            formData.append('filePath', 'models/' + username + '/' + modelName.slice(0, -4) + '.glb');
